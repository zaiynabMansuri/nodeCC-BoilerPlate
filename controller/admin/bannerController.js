 ...
 * @return {obj} : created Banner. {status, message, data}
 */ 
 ...
 * @return {obj} : found Banner(s). {status, message, data}
 */
const findAllBanner = async (req,res) => {
  try {
    ...
    if (req.body.isCountOnly){
      let totalRecords = await dbService.countDocument(Banner, query);
    ...
    let result = await dbService.getAllDocuments( Banner,query,options);
 ...
 * @param {obj} res : response contains updated documents of Banner.
 * @return {obj} : number of deactivated documents of Banner. {status, message, data}
 */
const softDeleteManyBanner = async (req,res) => {
  try {
    ...
    let data = await dbService.bulkUpdate(Banner,query, updateBody);
    if (!data) {
      return res.recordNotFound();
    }
 ...
 * @return {obj} : created Banners. {status, message, data}
 */
 ...
 * @param {obj} res : response of updated Banners.
 * @return {obj} : updated Banners. {status, message, data}
 */
...
const deleteManyBanner = async (req, res) => {
  try {
    ...
    const query = { '_id':{ '$in':ids } };
    let result = await dbService.deleteMany(Banner,query);
    return res.ok({ data :result });
  } catch (error){
    return res.failureResponse(); 
  }
 ...
 * @param {obj} res : response contains updated document of Banner.
 * @return {obj} : deactivated Banner. {status, message, data}
 */
const softDeleteBanner = async (req,res) => {
  try {
    let query = { _id:req.params.id };
    const updateBody = { isDeleted: true, };
    let result = await dbService.findOneAndUpdateDocument(Banner, query, updateBody,{ new:true });
    if (!result){
      return res.recordNotFound();
    }
    return  res.ok({ data:result });
 ...
 * @return {obj} : found Banner. {status, message, data}
 */
const getBanner = async (req,res) => {
  try {
    ...
    let result = await dbService.getSingleDocument(Banner,query, options);
    if (result){
 ...
 * @return {obj} : deleted Banner. {status, message, data}
 */
const deleteBanner = async (req,res) => {
  try {
    let query = { _id:req.params.id };
    const result = await dbService.findOneAndDeleteDocument(Banner, query);
    if (result){
      return  res.ok({ data :result });
    }
    return res.recordNotFound();
