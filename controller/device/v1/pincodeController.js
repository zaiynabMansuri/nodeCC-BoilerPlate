 ...
 * @return {obj} : created Pincode. {status, message, data}
 */ 
const addPincode = async (req, res) => {
  try {
    ...
    let data = new Pincode({
      ...req.body
      ,addedBy:req.user.id
    });
    let result = await dbService.createDocument(Pincode,data);
 ...
 * @return {obj} : found Pincode(s). {status, message, data}
 */
const findAllPincode = async (req,res) => {
  try {
    ...
    if (req.body.isCountOnly){
      let totalRecords = await dbService.countDocument(Pincode, query);
    ...
    let result = await dbService.getAllDocuments( Pincode,query,options);
...
const getPincodeCount = async (req,res) => {
  try {
    ...
    let result = await dbService.countDocument(Pincode,where);
 ...
 */
const getPincodeByAggregate = async (req,res)=>{
  try {
    let result = await dbService.getDocumentByAggregation(Pincode,req.body);
    if (result){
      return res.ok({ data :result });
    }
    return res.recordNotFound();
 ...
 * @param {obj} res : response contains updated documents of Pincode.
 * @return {obj} : number of deactivated documents of Pincode. {status, message, data}
 */
const softDeleteManyPincode = async (req,res) => {
  try {
    ...
    let data = await dbService.bulkUpdate(Pincode,query, updateBody);
    if (!data) {
      return res.recordNotFound();
    }
 ...
 * @return {obj} : created Pincodes. {status, message, data}
 */
const bulkInsertPincode = async (req,res)=>{
  try {
    ...
    let result = await dbService.bulkInsert(Pincode,data);
    return  res.ok({ data :result });
  } catch (error){
    if (error.name === 'ValidationError'){
      return res.validationError({ message : `Invalid Data, Validation Failed at ${ error.message}` });
 ...
 * @param {obj} res : response of updated Pincodes.
 * @return {obj} : updated Pincodes. {status, message, data}
 */
const bulkUpdatePincode = async (req,res)=>{
  try {
    ...
    if ( typeof req.body.data === 'object' && req.body.data !== null) {
      ...
      let result = await dbService.bulkUpdate(Pincode,filter,data);
      if (!result){
        return res.recordNotFound();
      }
      return  res.ok({ data :result });
...
const deleteManyPincode = async (req, res) => {
  try {
    ...
    const query = { '_id':{ '$in':ids } };
    let result = await dbService.deleteMany(Pincode,query);
    return res.ok({ data :result });
  } catch (error){
    return res.failureResponse(); 
  }
 ...
 * @param {obj} res : response contains updated document of Pincode.
 * @return {obj} : deactivated Pincode. {status, message, data}
 */
const softDeletePincode = async (req,res) => {
  try {
    let query = { _id:req.params.id };
    const updateBody = { isDeleted: true, };
    let result = await dbService.findOneAndUpdateDocument(Pincode, query, updateBody,{ new:true });
    if (!result){
      return res.recordNotFound();
    }
    return  res.ok({ data:result });
...
const partialUpdatePincode = async (req,res) => {
  try {
    ...
    let result = await dbService.findOneAndUpdateDocument(Pincode, query, data,{ new:true });
    if (!result) {
      return res.recordNotFound();
    }
    return res.ok({ data:result });
 ...
 * @return {obj} : found Pincode. {status, message, data}
 */
const getPincode = async (req,res) => {
  try {
    ...
    let result = await dbService.getSingleDocument(Pincode,query, options);
    if (result){
 ...
 * @return {obj} : deleted Pincode. {status, message, data}
 */
const deletePincode = async (req,res) => {
  try {
    let query = { _id:req.params.id };
    const result = await dbService.findOneAndDeleteDocument(Pincode, query);
    if (result){
      return  res.ok({ data :result });
    }
    return res.recordNotFound();
