 ...
 * @return {obj} : created City. {status, message, data}
 */ 
const addCity = async (req, res) => {
  try {
    ...
    let result = await dbService.createDocument(City,data);
 ...
 * @return {obj} : found City(s). {status, message, data}
 */
const findAllCity = async (req,res) => {
  try {
    ...
    if (req.body.isCountOnly){
      let totalRecords = await dbService.countDocument(City, query);
    ...
    let result = await dbService.getAllDocuments( City,query,options);
...
const getCityCount = async (req,res) => {
  try {
    ...
    let result = await dbService.countDocument(City,where);
 ...
 */
const getCityByAggregate = async (req,res)=>{
  try {
    let result = await dbService.getDocumentByAggregation(City,req.body);
    if (result){
      return res.ok({ data :result });
    }
    return res.recordNotFound();
 ...
 * @param {obj} res : response contains updated documents of City.
 * @return {obj} : number of deactivated documents of City. {status, message, data}
 */
 ...
 * @return {obj} : created Citys. {status, message, data}
 */
const bulkInsertCity = async (req,res)=>{
  try {
    ...
    let result = await dbService.bulkInsert(City,data);
    return  res.ok({ data :result });
  } catch (error){
    if (error.name === 'ValidationError'){
      return res.validationError({ message : `Invalid Data, Validation Failed at ${ error.message}` });
 ...
 * @param {obj} res : response of updated Citys.
 * @return {obj} : updated Citys. {status, message, data}
 */
const bulkUpdateCity = async (req,res)=>{
  try {
    ...
    if ( typeof req.body.data === 'object' && req.body.data !== null) {
      ...
      let result = await dbService.bulkUpdate(City,filter,data);
      if (!result){
        return res.recordNotFound();
      }
      return  res.ok({ data :result });
...
const deleteManyCity = async (req, res) => {
  try {
    ...
    if (req.body.isWarning) {
      let result = await deleteDependentService.countCity(query);
      return res.ok({ data :result }); 
    }
    else {
 ...
 * @param {obj} res : response contains updated document of City.
 * @return {obj} : deactivated City. {status, message, data}
 */
...
const partialUpdateCity = async (req,res) => {
  try {
    ...
    let result = await dbService.findOneAndUpdateDocument(City, query, data,{ new:true });
    if (!result) {
      return res.recordNotFound();
    }
    return res.ok({ data:result });
 ...
 * @return {obj} : found City. {status, message, data}
 */
const getCity = async (req,res) => {
  try {
    ...
    let result = await dbService.getSingleDocument(City,query, options);
    if (result){
 ...
 * @return {obj} : deleted City. {status, message, data}
 */
